...
def important_region(imge, vertices):
    ...
    return masked_image
def draw_lines(imge, lines):
    imge = ny.copy(imge)
    blank_image = ny.zeros((imge.shape[0],imge.shape[1], 3), dtype=ny.uint8)
...
lines = cv2.HoughLinesP(masked_image,rho=6,theta=ny.pi/180,threshold=160,lines=ny.array([]),minLineLength=40,maxLineGap=25)
